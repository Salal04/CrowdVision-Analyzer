...
def main(source=0, output=None, show=True):
    ...
    try:
        while True:
            ...
            # Collect person detections: [x1,y1,x2,y2,conf]
            person_boxes = []
            if hasattr(res, "boxes"):
                for box in res.boxes:
                    ...
                    if conf < MIN_CONFIDENCE:
                        continue
                    person_boxes.append([float(x1), float(y1), float(x2), float(y2), float(conf)])
            ...
            for track in tracker.tracks:
                ...
                mask_results = mask_GLass_detector(person_crop); # YOLOv8 detect
                res = mask_results[0] 
                ...
                for box in res.boxes:
                    ...
                    if "mask" in cls_name or "no_mask" in cls_name:
                ...
                if track.mask_Cord is not None and len(track.mask_Cord) > 0:
                    ...
                    cv2.putText(frame, track.mask_status, (mx1, my1-6), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 2)
...
if __name__ == "__main__":
    ...
    p.add_argument("--source", type=str, default="", help="0 for webcam or path to video")
